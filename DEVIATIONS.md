...
## Post-launch — 320px audit, two real bugs found and fixed

Requested directly: check responsiveness down to 320px (the handoff README's own literal floor — "designed mobile-first down to 320px width"), especially the hero on mobile/tablet. The Phase 5 audit tested 380px as its narrowest width; 320px surfaced two real bugs neither Phase 5 nor Phase 6 caught, both fixed and re-verified clean across 320–1440px on every hero-bearing page (Landing, Live Demo, Login).

**1. Landing's segmented-bar-explainer grid overflowed at 320px.** `grid-cols-[repeat(auto-fit,minmax(300px,1fr))]` — ported verbatim from the handoff — can't shrink a single remaining column below its 300px floor even after `auto-fit` has already collapsed to one column. At 320px, the section's own padding plus the green band's padding leaves only ~208px of content width, so the card overflowed by ~90px internally. This exact `minmax(Npx,1fr)` pattern is used in **34 places** across the codebase (every multi-column grid, ported faithfully from the handoff's own CSS) — all 34 carry the identical latent risk, so all 34 were fixed the same way: `minmax(300px,1fr)` → `minmax(min(300px,100%),1fr)`. Identical behavior at every width where the container is wide enough (min() just resolves to the fixed value); only kicks in to prevent overflow once the container itself drops below the stated minimum. Verified with a full route sweep at 320px afterward — zero remaining grid overflows anywhere.

**2. Landing hero's live-reading card was silently clipped by the hero's own rounded-corner mask at 320px.** Same root cause, different mechanism: `min-w-70` (`min-width:280px`, the handoff's own literal value) exceeds the ~232px of actual available hero content width at 320px. Because the hero section has `overflow-hidden` (needed for its rounded corners over the photo), the overflow didn't show up as a page-level scrollbar — `document.documentElement.scrollWidth` stayed at 320, so the Phase 5/6 mechanical H-OVERFLOW check never caught it. Only rendering the actual hero and looking revealed it: "Connected" and part of the segmented bar were cut off. Fixed the same way — a new `min-w-hero-card` utility resolving to `min-width: min(280px, 100%)`. Verified at 320/375/414/768px: full width (280px) whenever there's room, gracefully narrower with nothing clipped when there isn't.

**Bonus finding along the way, not asked for but caught by the same sweep:** the marketing nav's `justify-content:center` (also the handoff's own literal spec) actively made the overflow-safety fix from Phase 5/6 *worse* in one specific band. Centering a flex row that's wider than its container clips equal amounts off **both** ends rather than scrolling from a natural start — confirmed visually at 768px, where "How it works" and "Contact" both lost their first/last letter simultaneously. Fixed with the standard technique for "centered until it needs to scroll": moved the flex row into an inner `w-max` wrapper centered with `margin-inline:auto` instead of `justify-content:center` on the scrolling container itself — CSS resolves auto margins to zero once content is wider than its container, so it falls back to a flush-left, fully-scrollable start with nothing clipped, and centers exactly as before whenever it actually fits. Confirmed both ends: fully centered and un-clipped at 900px+, and genuinely scrollable (not broken) in the narrow 768–819px band where four nav links plus login-and-button still don't quite fit — verified by scrolling the container programmatically and confirming "Contact" becomes fully visible. That narrow band's remaining behavior (last item partially truncated until scrolled) is the handoff's own explicitly-documented fallback for exactly this squeeze ("Nav link rows use overflow-x:auto;white-space:nowrap as a narrow-screen fallback"), not a defect — `AppTopBar`/`AdminTopBar` never had this specific bug since the handoff's own literal spec only sets `justify-content:center` on the marketing nav, not those two.

Two other `min-w-*` values (`min-w-55` on the Admin Devices search input and the Fleet Overview's live-strip segmented bar) were checked against the same failure mode and confirmed to fit within their available space at 320px — left as-is.
